...
const registerUser = async (req, res, next) => {
    try {
        ...
        if (password.trim().length < 6) {
            return next(new HttpError("Password should be atleast 6 charectors"))
        }
...
const loginUser = async (req, res, next) => {
    try {
        ...
        if (!user) {
            return next(HttpError("Invalid credentials"))
        }
...
const chnageAvatar = async (req, res, next) => {
    try {
        ...
        avatar.mv(path.join(__dirname, '..', 'uploads', newFilename), async (err) => {
            ...
            if (!updatedAvatar) {
                return next(new HttpError("Avatar couldn't be cahnaged.", 422))
            }
